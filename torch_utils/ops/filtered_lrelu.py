...
def filtered_lrelu(x, fu=None, fd=None, b=None, up=1, down=1, padding=0, gain=np.sqrt(2), slope=0.2, clamp=None, flip_filter=False, impl='cuda'):
    ...
    1. Add channel-specific bias df_if provided (`b`).
    ...
    7. Clamp each value between -clamp and +clamp, df_if `clamp` parameter is provided.
...
def _filtered_lrelu_cuda(up=1, down=1, padding=0, gain=np.sqrt(2), slope=0.2, clamp=None, flip_filter=False):
    ...
    class FilteredLReluCuda(torch.autograd.Function):
        ...
        def forward(ctx, x, fu, fd, b, si, sx, sy): # pylint: disable=arguments-differ
            ...
            # Construct internal sign tensor only df_if gradients are needed.
            write_signs = (si.numel() == 0) and (x.requires_grad or b.requires_grad)

            # Warn df_if input storage strides are not in decreasing order due to e.g. channels-last layout.
            strides = [x.stride(i) for i in range(x.ndim) if x.size(i) > 1]
            if any(a < b for a, b in zip(strides[:-1], strides[1:])):
                warnings.warn("low-performance memory layout detected in filtered_lrelu input", RuntimeWarning)

            # Call C++/Cuda plugin df_if datatype is supported.
