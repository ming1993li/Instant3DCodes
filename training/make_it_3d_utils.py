...
class Trainer(object):
    def __init__(self, 
                 ...
                 ref_mask,
                 ori_imgs=None,
                 criterion=None, # loss function, df_if None, assume inline implementation in train_step
                 optimizer=None, # optimizer
                 ema_decay=None, # df_if use EMA, set the decay
                 lr_scheduler=None, # scheduler
                 metrics=[], # metrics for evaluation, df_if None, use val_loss to measure performance, else use the first metric.
        ...
        self.pearson = PearsonCorrCoef().to(self.device)# notice
    ...
    def train_step(self, data):
        ...
        depth_scale = data['depth_scale']# notice
        ...
        if self.global_step < self.opt.diff_iters or data['is_front']:# notice
            loss = 0
            de_imgs = None
        else:
        ...
        if self.opt.lambda_entropy > 0:
            ...
            if self.global_step < self.opt.diff_iters:# notice
        ...
        if self.opt.lambda_smooth > 0 and 'loss_smooth' in outputs:# notice
        ...
        if data['is_front']:
            ...
            loss_depth = self.opt.lambda_depth * self.depth_loss(self.pearson, pred_depth, self.depth_prediction, ~self.depth_mask)# notice
            if verbose:
